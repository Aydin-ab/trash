...
class Net(nn.Module):
    ...
    def forward(self, x, profile= False):
        x = self.conv1(x)
        x = F.relu(x)
        if self.first_forward and profile:
            NCU.start()
        x = self.conv2(x)
        if self.first_forward and profile: 
            NCU.stop()
...
def train(args, model, device, train_loader, optimizer, epoch, profile= False):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        optimizer.zero_grad()
        output = model(data, profile= True)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        if epoch == 5 :
            train(args, model, device, train_loader, optimizer, epoch, profile= True)
        else :
